 ...
 * @constructor
 */
function SessionToken({expireTime, redisKeyPrefix, redis, subReis,crontabStr,maxSize,useLru,clusteId,subscribeCallback,cacheWriteCallback}) {
...
SessionToken.prototype._addCache = function(token,value,expireTime=0) {
    ...
    if (this.maxSize > 0) {
        ...
        slogger.warn('maxsize overflow and not use lru algorithm');
    } else {
        this._saveTokenToMem(token,value,expireTime);
    }
};
