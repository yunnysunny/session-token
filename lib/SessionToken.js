 ...
 * 
 * @param {Number=} expireTime Expiration time in second, default is 0, which will be not expired, but when you set the paramter of `option.crontabStr`, it will be cleared at some time still.
 ...
 * @param {Number=} maxSize The max size of the cache in memory.
 * @param {Boolean=} useLru Whether to use LRU algorithm to delete the elder elements. It only takes effect when `option.maxSize` is greater than zero.
 ...
 * @constructor
 */
function SessionToken({expireTime, redisKeyPrefix, redis, subReis,crontabStr,maxSize,useLru,clusteId}) {
