...
SessionToken.prototype._saveTokenToStorage = function(token, value, expireTime=0, callback = function() {}) {
    ...
    if (this.expireTime && expireTime !== -1) {
        data.expire = new Date().getTime() + (expireTime || this.expireTime) * 1000 * this._memLifecycleRatio;
    } else {
        
    }
    const _this = this;
    this._cacheInstance.save(token, data, function(err, saved) {
        _this._cacheWriteCallback(err, token, data, saved);
        if (saved) {
    ...
    async.waterfall([
        ...
        function (key, valueStr, str, callback) {
            ...
            const setCallback = function (err) {
                ...
                _self._saveTokenToStorage(str, value);
                callback(false, str);
...
SessionToken.prototype.update = function (token, value, callback, ignoreRefresh) {
    ...
    const setCallback = function (err) {
        ...
        _self._cacheInstance.refresh(token, value);
        // _self._saveTokenToStorage(token, value, -1, function(err, saved) {
        //     if (err) {
        //         slogger.warn('save to local failed', err);
        //     }
        //     callback(false);
        // });
        callback(false);
...
SessionToken.prototype.refresh = function (token, callback) {
    ...
    this.redis.expire([keyName, expireTime], function (err) {
        ...
        callback(false);
        //TODO refresh storage expired time
    });
};
